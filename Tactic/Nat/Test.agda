...
simple n = quoteGoal g in unquote (auto g)

test₁ : ∀ n m → n + 3 * m + (20 + n) * n ≡ m + 21 * n + (m + n * n + m)
test₁ n m = quoteGoal g in unquote (auto g)

test₂ : ∀ x y → (x + y) ^ 3 ≡ x ^ 3 + 3 * x ^ 2 * y + 3 * x * y ^ 2 + y ^ 3
test₂ x y = quoteGoal g in unquote (auto g)
...
-- test₅ x = quoteGoal g in unquote (auto g)

-- foo : (x y : Bool) → x ≡ (y && false)
-- foo x y = quoteGoal g in unquote (auto g)

-- bad : ∀ x y → x + y ≡ x * 2
-- bad x y = quoteGoal g in unquote (auto g)
